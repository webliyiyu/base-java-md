...
1. **原子性（Atomicity）**：确保事务中包含的一组操作要么全部执行成功，要么完全不执行，保持数据操作的不可分割性。即使发生错误，系统也会回滚到事务开始前的状态，以此来保证事务作为一个整体被执行。
2. **一致性（Consistency）**：确保事务将数据库从一个一致状态转换到另一个一致状态。在事务开始之前和结束之后，数据库的完整性约束必须始终得到满足。
3. **隔离性（Isolation）**：确保并发执行的事务不会互相干扰，每个事务都感觉像是在独立地执行一样。隔离性防止了事务之间的交互直到它们完成执行，并防止了由于事务并行处理引起的数据不一致问题。
4. **持久性（Durability）**：一旦事务被提交，它对数据库的修改就是永久性的。即使在事务提交后发生系统故障，这些修改也应该保持，从而确保了事务的效果能够抵御系统崩溃和其他类型的故障。
